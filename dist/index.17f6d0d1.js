...
})({"hemPM":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "9a036c1417f6d0d1";
...
},{}],"9yajy":[function(require,module,exports) {
...
var tns = function(options) {
    ...
    var CALC = tnsStorage['tC'] ? _checkStorageValueJs.checkStorageValue(tnsStorage['tC']) : _setLocalStorageJs.setLocalStorage(tnsStorage, 'tC', _calcJs.calc(), localStorageAccess), PERCENTAGELAYOUT = tnsStorage['tPL'] ? _checkStorageValueJs.checkStorageValue(tnsStorage['tPL']) : _setLocalStorageJs.setLocalStorage(tnsStorage, 'tPL', _percentageLayoutJs.percentageLayout(), localStorageAccess), CSSMQ = tnsStorage['tMQ'] ? _checkStorageValueJs.checkStorageValue(tnsStorage['tMQ']) : _setLocalStorageJs.setLocalStorage(tnsStorage, 'tMQ', _mediaquerySupportJs.mediaquerySupport(), localStorageAccess), TRANSFORM = tnsStorage['tTf'] ? _checkStorageValueJs.checkStorageValue(tnsStorage['tTf']) : _setLocalStorageJs.setLocalStorage(tnsStorage, 'tTf', _whichPropertyJs.whichProperty('transform'), localStorageAccess), HAS3DTRANSFORMS = tnsStorage['t3D'] ? _checkStorageValueJs.checkStorageValue(tnsStorage['t3D']) : _setLocalStorageJs.setLocalStorage(tnsStorage, 't3D', _has3DTransformsJs.has3DTransforms(TRANSFORM), localStorageAccess), TRANSITIONDURATION = tnsStorage['tTDu'] ? _checkStorageValueJs.checkStorageValue(tnsStorage['tTDu']) : _setLocalStorageJs.setLocalStorage(tnsStorage, 'tTDu', _whichPropertyJs.whichProperty('transitionDuration'), localStorageAccess), TRANSITIONDELAY = tnsStorage['tTDe'] ? _checkStorageValueJs.checkStorageValue(tnsStorage['tTDe']) : _setLocalStorageJs.setLocalStorage(tnsStorage, 'tTDe', _whichPropertyJs.whichProperty('transitionDelay'), localStorageAccess), ANIMATIONDURATION = tnsStorage['tADu'] ? _checkStorageValueJs.checkStorageValue(tnsStorage['tADu']) : _setLocalStorageJs.setLocalStorage(tnsStorage, 'tADu', _whichPropertyJs.whichProperty('animationDuration'), localStorageAccess), ANIMATIONDELAY = tnsStorage['tADe'] ? _checkStorageValueJs.checkStorageValue(tnsStorage['tADe']) : _setLocalStorageJs.setLocalStorage(tnsStorage, 'tADe', _whichPropertyJs.whichProperty('animationDelay'), localStorageAccess), TRANSITIONEND = tnsStorage['tTE'] ? _checkStorageValueJs.checkStorageValue(tnsStorage['tTE']) : _setLocalStorageJs.setLocalStorage(tnsStorage, 'tTE', _getEndPropertyJs.getEndProperty(TRANSITIONDURATION, 'Transition'), localStorageAccess), ANIMATIONEND = tnsStorage['tAE'] ? _checkStorageValueJs.checkStorageValue(tnsStorage['tAE']) : _setLocalStorageJs.setLocalStorage(tnsStorage, 'tAE', _getEndPropertyJs.getEndProperty(ANIMATIONDURATION, 'Animation'), localStorageAccess);
    ...
    if (responsive) {
        ...
        if (0 in responsive) {
            options = _extendJs.extend(options, responsive[0]);
            delete responsive[0];
        }
        var responsiveTem = {
        };
    ...
    var autoWidth = options.autoWidth, fixedWidth = getOption('fixedWidth'), edgePadding = getOption('edgePadding'), gutter = getOption('gutter'), viewport = getViewportWidth(), center = getOption('center'), items = !autoWidth ? Math.floor(getOption('items')) : 1, slideBy = getOption('slideBy'), viewportMax = options.viewportMax || options.fixedWidthViewportWidth, arrowKeys = getOption('arrowKeys'), speed1 = getOption('speed'), rewind = options.rewind, loop = rewind ? false : options.loop, autoHeight = getOption('autoHeight'), controls = getOption('controls'), controlsText = getOption('controlsText'), nav = getOption('nav'), touch = getOption('touch'), mouseDrag = getOption('mouseDrag'), autoplay = getOption('autoplay'), autoplayTimeout = getOption('autoplayTimeout'), autoplayText = getOption('autoplayText'), autoplayHoverPause = getOption('autoplayHoverPause'), autoplayResetOnVisibility = getOption('autoplayResetOnVisibility'), sheet = _createStyleSheetJs.createStyleSheet(null, getOption('nonce')), lazyload = options.lazyload, lazyloadSelector = options.lazyloadSelector, slidePositions, slideItemsOut = [], cloneCount = loop ? getCloneCountForLoop() : 0, slideCountNew = !carousel ? slideCount + cloneCount : slideCount + cloneCount * 2, hasRightDeadZone = (fixedWidth || autoWidth) && !loop ? true : false, rightBoundary = fixedWidth ? getRightBoundary() : null, updateIndexBeforeTransform = !carousel || !loop ? true : false, // transform
    ...
    }(), index1 = getStartIndex(getOption('startIndex')), indexCached = index1, displayIndex = getCurrentSlide(), indexMin = 0, indexMax = !autoWidth ? getIndexMax() : null, // resize
    resizeTimer, preventActionWhenRunning = options.preventActionWhenRunning, swipeAngle = options.swipeAngle, moveDirectionExpected = swipeAngle ? '?' : true, running = false, onInit = options.onInit, events = new _eventsJs.Events(), // id, class
    newContainerClasses = ' tns-slider tns-' + options.mode, slideId = container.id || _getSlideIdJs.getSlideId(), disable = getOption('disable'), disabled = false, freezable = options.freezable, freeze = freezable && !autoWidth ? getFreeze() : false, frozen = false, controlsEvents = {
    ...
    }, translateInit, disX, disY, panStart = false, rafIndex, getDist = horizontal ? function(a, b) {
        return a.x - b.x;
    } : function(a, b) {
        return a.y - b.y;
    };
    ...
    function initStructure() {
        ...
        // before clone slides
        _forEachJs.forEach(slideItems, function(item, i) {
            _addClassJs.addClass(item, 'tns-item');
            if (!item.id) item.id = slideId + '-item' + i;
            if (!carousel && animateNormal) _addClassJs.addClass(item, animateNormal);
            _setAttrsJs.setAttrs(item, {
                'aria-hidden': 'true',
                'tabindex': '-1'
            });
        });
        ...
        if (cloneCount) {
            ...
            for(var j = cloneCount; j--;){
                ...
                _addClassJs.addClass(cloneFirst, slideClonedClass);
                _removeAttrsJs.removeAttrs(cloneFirst, 'id');
                fragmentAfter.insertBefore(cloneFirst, fragmentAfter.firstChild);
                if (carousel) {
                    var cloneLast = slideItems[slideCount - 1 - num].cloneNode(true);
                    _addClassJs.addClass(cloneLast, slideClonedClass);
                    _removeAttrsJs.removeAttrs(cloneLast, 'id');
    ...
    function initSliderTransform() {
        ...
        if (hasOption('autoHeight') || autoWidth || !horizontal) {
            ...
            _forEachJs.forEach(imgs, function(img) {
                var src = img.src;
                if (!lazyload) {
                    // not data img
                    if (src && src.indexOf('data:image') < 0) {
                        img.src = '';
                        _addEventsJs.addEvents(img, imgEvents);
                        _addClassJs.addClass(img, 'loading');
                        img.src = src;
                    // data img
                    } else imgLoaded(img);
                }
            ...
            // set imgsComplete
            _rafJs.raf(function() {
                imgsLoadedCheck(_arrayFromNodeListJs.arrayFromNodeList(imgs), function() {
                    imgsComplete = true;
                });
            });
            // reset imgs for auto height: check visible imgs only
            if (hasOption('autoHeight')) imgs = getImageArray(index1, Math.min(index1 + items - 1, slideCountNew - 1));
            lazyload ? initSliderTransformStyleCheck() : _rafJs.raf(function() {
                imgsLoadedCheck(_arrayFromNodeListJs.arrayFromNodeList(imgs), initSliderTransformStyleCheck);
            });
        } else {
    ...
    function initSheet() {
        ...
        if (!carousel) for(var i1 = index1, l = index1 + Math.min(slideCount, items); i1 < l; i1++){
            ...
            _addClassJs.addClass(item, animateIn);
            _removeClassJs.removeClass(item, animateNormal);
        }
        // #### LAYOUT
        ...
        if (horizontal) {
            if (PERCENTAGELAYOUT || autoWidth) {
                _addCSSRuleJs.addCSSRule(sheet, '#' + slideId + ' > .tns-item', 'font-size:' + win.getComputedStyle(slideItems[0]).fontSize + ';', _getCssRulesLengthJs.getCssRulesLength(sheet));
                _addCSSRuleJs.addCSSRule(sheet, '#' + slideId, 'font-size:0;', _getCssRulesLengthJs.getCssRulesLength(sheet));
            } else if (carousel) _forEachJs.forEach(slideItems, function(slide, i) {
        ...
        if (CSSMQ) {
            ...
            if (TRANSITIONDURATION) {
                ...
                _addCSSRuleJs.addCSSRule(sheet, '#' + slideId + '-mw', str, _getCssRulesLengthJs.getCssRulesLength(sheet));
            }
            // inner wrapper styles
            str = getInnerWrapperStyles(options.edgePadding, options.gutter, options.fixedWidth, options.speed, options.autoHeight);
            _addCSSRuleJs.addCSSRule(sheet, '#' + slideId + '-iw', str, _getCssRulesLengthJs.getCssRulesLength(sheet));
            // container styles
            if (carousel) {
                str = horizontal && !autoWidth ? 'width:' + getContainerWidth(options.fixedWidth, options.gutter, options.items) + ';' : '';
                if (TRANSITIONDURATION) str += getTransitionDurationStyle(speed1);
                _addCSSRuleJs.addCSSRule(sheet, '#' + slideId, str, _getCssRulesLengthJs.getCssRulesLength(sheet));
            }
            // slide styles
            ...
            if (str) _addCSSRuleJs.addCSSRule(sheet, '#' + slideId + ' > .tns-item', str, _getCssRulesLengthJs.getCssRulesLength(sheet));
    ...
    function initTools() {
        ...
        if (hasAutoplay) {
            ...
            if (autoplayButton) _setAttrsJs.setAttrs(autoplayButton, {
                'data-action': txt
            });
            ...
            // add event
            if (autoplayButton) _addEventsJs.addEvents(autoplayButton, {
                'click': toggleAutoplay
            });
            if (autoplay) {
                startAutoplay();
                if (autoplayHoverPause) _addEventsJs.addEvents(container, hoverEvents);
                if (autoplayResetOnVisibility) _addEventsJs.addEvents(container, visibilityEvent);
            }
        }
        // == navInit ==
        if (hasNav) {
            ...
            if (navContainer) {
                _setAttrsJs.setAttrs(navContainer, {
                    'aria-label': 'Carousel Pagination'
                });
                navItems = navContainer.children;
                _forEachJs.forEach(navItems, function(item, i) {
                    _setAttrsJs.setAttrs(item, {
                        'data-nav': i,
                        'tabindex': '-1',
            ...
            if (TRANSITIONDURATION) {
                ...
                _addCSSRuleJs.addCSSRule(sheet, '[aria-controls^=' + slideId + '-item]', str, _getCssRulesLengthJs.getCssRulesLength(sheet));
            }
            _setAttrsJs.setAttrs(navItems[navCurrentIndex], {
                'aria-label': navStr + (navCurrentIndex + 1) + navStrCurrent
            });
            _removeAttrsJs.removeAttrs(navItems[navCurrentIndex], 'tabindex');
            _addClassJs.addClass(navItems[navCurrentIndex], navActiveClass);
            // add events
            _addEventsJs.addEvents(navContainer, navEvents);
        }
        // == controlsInit ==
        if (hasControls) {
            ...
            if (options.controlsContainer) _setAttrsJs.setAttrs(controlsContainer, {
                'aria-label': 'Carousel Navigation',
                'tabindex': '0'
            });
            if (options.controlsContainer || options.prevButton && options.nextButton) _setAttrsJs.setAttrs([
                prevButton,
                nextButton
            ], {
                'aria-controls': slideId,
            ...
            if (options.controlsContainer || options.prevButton && options.nextButton) {
                _setAttrsJs.setAttrs(prevButton, {
                    'data-controls': 'prev'
                });
                _setAttrsJs.setAttrs(nextButton, {
                    'data-controls': 'next'
                });
            }
            ...
            // add events
            if (controlsContainer) _addEventsJs.addEvents(controlsContainer, controlsEvents);
            else {
                _addEventsJs.addEvents(prevButton, controlsEvents);
                _addEventsJs.addEvents(nextButton, controlsEvents);
            }
        }
    ...
    function initEvents() {
        ...
        if (carousel && TRANSITIONEND) {
            ...
            _addEventsJs.addEvents(container, eve);
        }
        if (touch) _addEventsJs.addEvents(container, touchEvents, options.preventScrollOnTouch);
        if (mouseDrag) _addEventsJs.addEvents(container, dragEvents);
        if (arrowKeys) _addEventsJs.addEvents(doc, docmentKeydownEvent);
        if (nested === 'inner') events.on('outerResized', function() {
            resizeTasks();
            events.emit('innerLoaded', info());
        });
        else if (responsive || fixedWidth || autoWidth || autoHeight || !horizontal) _addEventsJs.addEvents(win, {
            'resize': onResize
        });
        if (autoHeight) {
    ...
    function destroy() {
        ...
        _removeEventsJs.removeEvents(win, {
            'resize': onResize
        });
        // arrowKeys, controls, nav
        if (arrowKeys) _removeEventsJs.removeEvents(doc, docmentKeydownEvent);
        if (controlsContainer) _removeEventsJs.removeEvents(controlsContainer, controlsEvents);
        if (navContainer) _removeEventsJs.removeEvents(navContainer, navEvents);
        // autoplay
        _removeEventsJs.removeEvents(container, hoverEvents);
        _removeEventsJs.removeEvents(container, visibilityEvent);
        if (autoplayButton) _removeEventsJs.removeEvents(autoplayButton, {
            'click': toggleAutoplay
        });
        ...
        if (carousel && TRANSITIONEND) {
            ...
            eve[TRANSITIONEND] = onTransitionEnd;
            _removeEventsJs.removeEvents(container, eve);
        }
        if (touch) _removeEventsJs.removeEvents(container, touchEvents);
        if (mouseDrag) _removeEventsJs.removeEvents(container, dragEvents);
    ...
    function resizeTasks(e) {
        ...
        if (arrowKeys !== arrowKeysTem) arrowKeys ? _addEventsJs.addEvents(doc, docmentKeydownEvent) : _removeEventsJs.removeEvents(doc, docmentKeydownEvent);
        if (controls !== controlsTem) {
            if (controls) {
                if (controlsContainer) _showElementJs.showElement(controlsContainer);
                else {
                    if (prevButton) _showElementJs.showElement(prevButton);
                    if (nextButton) _showElementJs.showElement(nextButton);
                }
            } else if (controlsContainer) _hideElementJs.hideElement(controlsContainer);
            else {
                if (prevButton) _hideElementJs.hideElement(prevButton);
                if (nextButton) _hideElementJs.hideElement(nextButton);
            }
        }
        if (nav !== navTem) {
            if (nav) {
                _showElementJs.showElement(navContainer);
                updateNavVisibility();
            } else _hideElementJs.hideElement(navContainer);
        }
        if (touch !== touchTem) touch ? _addEventsJs.addEvents(container, touchEvents, options.preventScrollOnTouch) : _removeEventsJs.removeEvents(container, touchEvents);
        if (mouseDrag !== mouseDragTem) mouseDrag ? _addEventsJs.addEvents(container, dragEvents) : _removeEventsJs.removeEvents(container, dragEvents);
        if (autoplay !== autoplayTem) {
            if (autoplay) {
                if (autoplayButton) _showElementJs.showElement(autoplayButton);
                if (!animating && !autoplayUserPaused) startAutoplay();
            } else {
                if (autoplayButton) _hideElementJs.hideElement(autoplayButton);
                if (animating) stopAutoplay();
            }
        }
        if (autoplayHoverPause !== autoplayHoverPauseTem) autoplayHoverPause ? _addEventsJs.addEvents(container, hoverEvents) : _removeEventsJs.removeEvents(container, hoverEvents);
        if (autoplayResetOnVisibility !== autoplayResetOnVisibilityTem) autoplayResetOnVisibility ? _addEventsJs.addEvents(doc, visibilityEvent) : _removeEventsJs.removeEvents(doc, visibilityEvent);
        if (bpChanged) {
        ...
        if (!disable && !freeze) {
            ...
            if (bpChanged && !CSSMQ) {
                ...
                if (horizontal) {
                    ...
                    // add new styles
                    _removeCSSRuleJs.removeCSSRule(sheet, _getCssRulesLengthJs.getCssRulesLength(sheet) - 1);
                    _addCSSRuleJs.addCSSRule(sheet, '#' + slideId + ' > .tns-item', str, _getCssRulesLengthJs.getCssRulesLength(sheet));
                }
            }
            // auto height
    ...
    function disableUI() {
        if (!autoplay && autoplayButton) _hideElementJs.hideElement(autoplayButton);
        if (!nav && navContainer) _hideElementJs.hideElement(navContainer);
        if (!controls) {
            if (controlsContainer) _hideElementJs.hideElement(controlsContainer);
            else {
                if (prevButton) _hideElementJs.hideElement(prevButton);
                if (nextButton) _hideElementJs.hideElement(nextButton);
            }
    ...
    function enableUI() {
        if (autoplay && autoplayButton) _showElementJs.showElement(autoplayButton);
        if (nav && navContainer) _showElementJs.showElement(navContainer);
        if (controls) {
            if (controlsContainer) _showElementJs.showElement(controlsContainer);
            else {
                if (prevButton) _showElementJs.showElement(prevButton);
    ...
    function freezeSlider() {
        ...
        if (cloneCount) {
            ...
            for(var i = cloneCount; i--;){
                if (carousel) _addClassJs.addClass(slideItems[i], str);
                _addClassJs.addClass(slideItems[slideCountNew - i - 1], str);
            }
        }
        // update tools
        disableUI();
    ...
    function unfreezeSlider() {
        ...
        if (cloneCount) {
            var str = 'tns-transparent';
            for(var i = cloneCount; i--;){
                if (carousel) _removeClassJs.removeClass(slideItems[i], str);
                _removeClassJs.removeClass(slideItems[slideCountNew - i - 1], str);
            }
        }
        // update tools
        enableUI();
    ...
    function disableSlider() {
        ...
        _removeAttrsJs.removeAttrs(container, [
            'style'
        ]);
        if (loop) for(var j = cloneCount; j--;){
            if (carousel) _hideElementJs.hideElement(slideItems[j]);
            _hideElementJs.hideElement(slideItems[slideCountNew - j - 1]);
        }
        // vertical slider
        if (!horizontal || !carousel) _removeAttrsJs.removeAttrs(innerWrapper, [
            'style'
        ]);
        // gallery
        if (!carousel) for(var i = index1, l = index1 + slideCount; i < l; i++){
            var item = slideItems[i];
            _removeAttrsJs.removeAttrs(item, [
                'style'
            ]);
            _removeClassJs.removeClass(item, animateIn);
            _removeClassJs.removeClass(item, animateNormal);
        }
        // update tools
        disableUI();
    ...
    function enableSlider() {
        ...
        doContainerTransformSilent();
        if (loop) for(var j = cloneCount; j--;){
            if (carousel) _showElementJs.showElement(slideItems[j]);
            _showElementJs.showElement(slideItems[slideCountNew - j - 1]);
        }
        // gallery
        if (!carousel) for(var i = index1, l = index1 + slideCount; i < l; i++){
            var item = slideItems[i], classN = i < index1 + items ? animateIn : animateNormal;
            item.style.left = (i - index1) * 100 / items + '%';
            _addClassJs.addClass(item, classN);
        }
        // update tools
        enableUI();
        disabled = false;
    ...
    function getVisibleSlideRange(val) {
        ...
        var start = index1, end, rangestart, rangeend;
    ...
    function doLazyLoad() {
        if (lazyload && !disable) {
            ...
            getImageArray.apply(null, arg).forEach(function(img) {
                if (!_hasClassJs.hasClass(img, imgCompleteClass)) {
                    ...
                    _addEventsJs.addEvents(img, eve);
                    _addEventsJs.addEvents(img, imgEvents);
                    // update src
                    img.src = _getAttrJs.getAttr(img, 'data-src');
                    // update srcset
                    var srcset = _getAttrJs.getAttr(img, 'data-srcset');
    ...
    function imgLoaded(img) {
        _addClassJs.addClass(img, 'loaded');
        imgCompleted(img);
    }
    function imgFailed(img) {
        _addClassJs.addClass(img, 'failed');
        imgCompleted(img);
    }
    function imgCompleted(img) {
        _addClassJs.addClass(img, imgCompleteClass);
        _removeClassJs.removeClass(img, 'loading');
        _removeEventsJs.removeEvents(img, imgEvents);
    }
    function getImageArray(start, end, imgSelector) {
        var imgs = [];
        if (!imgSelector) imgSelector = 'img';
        while(start <= end){
            _forEachJs.forEach(slideItems[start].querySelectorAll(imgSelector), function(img) {
                imgs.push(img);
            });
            start++;
        }
    ...
    function imgsLoadedCheck(imgs, cb) {
        ...
        imgs.forEach(function(img, index) {
            ...
            if (_hasClassJs.hasClass(img, imgCompleteClass)) imgs.splice(index, 1);
        });
    ...
    function updateSlideStatus() {
        ...
        _forEachJs.forEach(slideItems, function(item, i) {
            // show slides
            if (i >= start && i <= end) {
                if (_hasAttrJs.hasAttr(item, 'aria-hidden')) {
                    _removeAttrsJs.removeAttrs(item, [
                        'aria-hidden',
                        'tabindex'
                    ]);
                    _addClassJs.addClass(item, slideActiveClass);
                }
            } else if (!_hasAttrJs.hasAttr(item, 'aria-hidden')) {
                _setAttrsJs.setAttrs(item, {
                    'aria-hidden': 'true',
                    'tabindex': '-1'
                });
                _removeClassJs.removeClass(item, slideActiveClass);
            }
        });
    }
    ...
    function updateGallerySlidePositions() {
        ...
        for(var i = slideCountNew; i--;){
            ...
            if (i >= index1 && i < l) {
                ...
                _addClassJs.addClass(item, 'tns-moving');
                item.style.left = (i - index1) * 100 / items + '%';
                _addClassJs.addClass(item, animateIn);
                _removeClassJs.removeClass(item, animateNormal);
            } else if (item.style.left) {
                item.style.left = '';
                _addClassJs.addClass(item, animateNormal);
                _removeClassJs.removeClass(item, animateIn);
            }
            // remove outlet animation
            _removeClassJs.removeClass(item, animateOut);
        }
        // removing '.tns-moving'
        setTimeout(function() {
            _forEachJs.forEach(slideItems, function(el) {
                _removeClassJs.removeClass(el, 'tns-moving');
            });
        }, 300);
    }
    // set tabindex on Nav
    function updateNavStatus() {
        ...
        if (nav) {
            ...
            if (navCurrentIndex !== navCurrentIndexCached) {
                ...
                _setAttrsJs.setAttrs(navPrev, {
                    'tabindex': '-1',
                    'aria-label': navStr + (navCurrentIndexCached + 1)
                });
                _removeClassJs.removeClass(navPrev, navActiveClass);
                _setAttrsJs.setAttrs(navCurrent, {
                    'aria-label': navStr + (navCurrentIndex + 1) + navStrCurrent
                });
                _removeAttrsJs.removeAttrs(navCurrent, 'tabindex');
                _addClassJs.addClass(navCurrent, navActiveClass);
    ...
    function animateSlide(number, classOut, classIn, isOut) {
        ...
        for(var i = number; i < l; i++){
            ...
            _removeClassJs.removeClass(item, classOut);
            _addClassJs.addClass(item, classIn);
    ...
    var transformCore = function() {
        return carousel ? function() {
            ...
            if (TRANSITIONDURATION || !speed1) {
                ...
                if (!speed1 || !_isVisibleJs.isVisible(container)) onTransitionEnd();
            } else // for old browser with non-zero duration
            _jsTransformJs.jsTransform(container, transformAttr, transformPrefix, transformPostfix, getContainerTransformValue(), speed1, onTransitionEnd);
            ...
            _removeEventsJs.removeEvents(slideItems[indexCached], eve);
            _addEventsJs.addEvents(slideItems[index1], eve);
            animateSlide(indexCached, animateIn, animateOut, true);
            animateSlide(index1, animateNormal, animateIn);
            // run fallback function manually
            // when transition or animation not supported / duration is 0
            if (!TRANSITIONEND || !ANIMATIONEND || !speed1 || !_isVisibleJs.isVisible(container)) onTransitionEnd();
        };
    }();
   ...
   */ function strTrans(str) {
    ...
    function goTo(targetIndex, e) {
        ...
        if (targetIndex === 'prev') onControlsClick(e, -1);
            ...
            if (getAbsIndex(index1) !== getAbsIndex(indexCached)) render(e);
        }
    }
    // on controls click
    ...
    function onNavClick(e) {
        ...
        while(target !== navContainer && !_hasAttrJs.hasAttr(target, 'data-nav'))target = target.parentNode;
        if (_hasAttrJs.hasAttr(target, 'data-nav')) {
            var navIndex = navClicked = Number(_getAttrJs.getAttr(target, 'data-nav')), targetIndexBase = fixedWidth || autoWidth ? navIndex * slideCount / pages : navIndex * items, targetIndex = navAsThumbnails ? navIndex : Math.min(Math.ceil(targetIndexBase), slideCount - 1);
            goTo(targetIndex, e);
    ...
    function updateAutoplayButton(action, txt) {
        _setAttrsJs.setAttrs(autoplayButton, {
            'data-action': action
        });
    ...
    function onNavKeydown(e) {
        ...
        if (!_hasAttrJs.hasAttr(curElement, 'data-nav')) return;
        // var code = e.keyCode,
        var keyIndex = [
            KEYS.LEFT,
            KEYS.RIGHT,
            KEYS.ENTER,
            KEYS.SPACE
        ].indexOf(e.keyCode), navIndex = Number(_getAttrJs.getAttr(curElement, 'data-nav'));
        if (keyIndex >= 0) {
            if (keyIndex === 0) {
    ...
    function getMoveDirectionExpected() {
        return _getTouchDirectionJs.getTouchDirection(_toDegreeJs.toDegree(lastPosition.y - initPosition.y, lastPosition.x - initPosition.x), swipeAngle) === options.axis;
    }
    ...
    function onPanMove(e) {
        if (panStart) {
            ...
            if (carousel) {
                if (!rafIndex) rafIndex = _rafJs.raf(function() {
                    panUpdate(e);
                });
            } else {
    ...
    function panUpdate(e) {
        ...
        _cafJs.caf(rafIndex);
        if (panStart) rafIndex = _rafJs.raf(function() {
            panUpdate(e);
        });
        if (moveDirectionExpected === '?') moveDirectionExpected = getMoveDirectionExpected();
        if (moveDirectionExpected) {
        ...
        if (panStart) {
            ...
            if (Math.abs(dist)) {
                ...
                if (!isTouchEvent(e1)) {
                    ...
                    _addEventsJs.addEvents(target, {
                        'click': function preventClick(e) {
                            preventDefaultBehavior(e);
                            _removeEventsJs.removeEvents(target, {
                                'click': preventClick
                            });
                        }
                    });
                }
                if (carousel) rafIndex = _rafJs.raf(function() {
                    if (horizontal && !autoWidth) {
                    ...
                    render(e1, dist);
                    events.emit(isTouchEvent(e1) ? 'touchEnd' : 'dragEnd', info(e1));
                });
                else if (moveDirectionExpected) onControlsClick(e1, dist > 0 ? -1 : 1);
            }
        }
        // reset
   ...
   */ function updateNavVisibility() {
        ...
        if (pages !== pagesCached) {
            var min = pagesCached, max = pages, fn = _showElementJs.showElement;
            if (pagesCached > pages) {
                min = pages;
                max = pagesCached;
                fn = _hideElementJs.hideElement;
            }
            while(min < max){
                fn(navItems[min]);
                min++;
    ...
    return {
        ...
        destroy: destroy,
        rebuild: function() {
            return tns(_extendJs.extend(options, optionsElements));
        }
    };
};
...
function calc() {
    var doc = document, body = _getBodyJs.getBody(), docOverflow = _setFakeBodyJs.setFakeBody(body), div = doc.createElement('div'), result = false;
    ...
    body.fake ? _resetFakeBodyJs.resetFakeBody(body, docOverflow) : div.remove();
    return result;
}
...
function percentageLayout() {
    ...
    var doc = document, body = _getBodyJs.getBody(), docOverflow = _setFakeBodyJs.setFakeBody(body), wrapper = doc.createElement('div'), outer = doc.createElement('div'), str = '', count = 70, perPage = 3, supported = false;
    ...
    body.fake ? _resetFakeBodyJs.resetFakeBody(body, docOverflow) : wrapper.remove();
    return supported;
}
...
function mediaquerySupport() {
    ...
    var doc = document, body = _getBodyJs.getBody(), docOverflow = _setFakeBodyJs.setFakeBody(body), div = doc.createElement('div'), style = doc.createElement('style'), rule = '@media all and (min-width:1px){.tns-mq-test{position:absolute}}', position;
    style.type = 'text/css';
...
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "classListSupport", ()=>_classListSupportJs.classListSupport
);
...
var addClass = _hasClassJs.classListSupport ? function(el, str) {
    if (!_hasClassJs.hasClass(el, str)) el.classList.add(str);
} : function(el, str) {
    if (!_hasClassJs.hasClass(el, str)) el.className += ' ' + str;
};
...
var removeClass = _hasClassJs.classListSupport ? function(el, str) {
    if (_hasClassJs.hasClass(el, str)) el.classList.remove(str);
} : function(el, str) {
    if (_hasClassJs.hasClass(el, str)) el.className = el.className.replace(str, '');
};
...
function setAttrs(els, attrs) {
    els = _isNodeListJs.isNodeList(els) || els instanceof Array ? els : [
        els
    ];
...
function has3DTransforms(tf) {
    ...
    var doc = document, body = _getBodyJs.getBody(), docOverflow = _setFakeBodyJs.setFakeBody(body), el = doc.createElement('p'), has3d, cssTF = tf.length > 9 ? '-' + tf.slice(0, -9).toLowerCase() + '-' : '';
    cssTF += 'transform';
    // Add it to the body to get the computed style
    body.insertBefore(el, null);
    el.style[tf] = 'translate3d(1px,1px,1px)';
    has3d = window.getComputedStyle(el).getPropertyValue(cssTF);
    body.fake ? _resetFakeBodyJs.resetFakeBody(body, docOverflow) : el.remove();
...
function addEvents(el, obj, preventScrolling) {
    for(var prop in obj){
        var option = [
            'touchstart',
            'touchmove'
        ].indexOf(prop) >= 0 && !preventScrolling ? _passiveOptionJs.passiveOption : false;
...
function removeEvents(el, obj) {
    for(var prop in obj){
        var option = [
            'touchstart',
            'touchmove'
        ].indexOf(prop) >= 0 ? _passiveOptionJs.passiveOption : false;
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["hemPM","9yajy"], "9yajy", "parcelRequire317f")

//# sourceMappingURL=index.17f6d0d1.js.map
